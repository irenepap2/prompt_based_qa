...
def compute_bert_score(gold, predicted):
    refs = []
    for references in gold.values():
        refs.append([ref["answer"] for ref in references])
    preds = [pred["answer"] for pred in predicted.values()]
...
def evaluate(gold, predicted):
    ...
    return {
        ...
        # "BertScore": compute_bert_score(gold, predicted),
...
if __name__ == "__main__":
    ...
    filename = args.predictions.split("/")[-1].replace(".jsonl", "")
    path = "/".join(args.predictions.split("/")[:-1])

    with open(f"{path}/{filename}_results.json", "w") as f:
